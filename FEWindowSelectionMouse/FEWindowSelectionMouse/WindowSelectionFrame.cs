namespace FreeEcho
{
    namespace FEWindowSelectionMouse
    {
        ...
        public class WindowSelectionFrame : System.IDisposable
        {
            ...
            {
                System.IntPtr selectedHwnd;

                if ((selectedHwnd = NativeMethods.WindowFromPoint(System.Windows.Forms.Control.MousePosition)) != System.IntPtr.Zero)
                {
                    if ((selectedHwnd = NativeMethods.GetAncestor(selectedHwnd, 2)) != System.IntPtr.Zero)       // 2 = GA_ROOT (親ウィンドウのルートウィンドウを取得の値)
                    {
                        if ((selectedHwnd != SelectedHwnd) && (selectedHwnd != FrameWindow.Handle))
                        {
                            SelectedHwnd = selectedHwnd;

                            RECT windowRect;
                            NativeMethods.GetWindowRect(selectedHwnd, out windowRect);
                            windowRect.right -= windowRect.left;
                            windowRect.bottom -= windowRect.top;

                            // 枠の位置とサイズを計算
                            WINDOWPLACEMENT windowPlacement;
                            NativeMethods.GetWindowPlacement(selectedHwnd, out windowPlacement);
                            System.Drawing.Rectangle settingFrameWindowRectangle = new();
                            if (windowPlacement.showCmd == 3)      // 3 = SW_SHOWMAXIMIZED
                            {
                                System.Drawing.Rectangle screenRectangle = System.Windows.Forms.Screen.FromHandle(selectedHwnd).WorkingArea;
                            ...
                            System.Drawing.Rectangle screen = System.Windows.Forms.Screen.FromHandle(selectedHwnd).Bounds;
